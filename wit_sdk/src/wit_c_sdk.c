...
/**
 * @param: ucIndex: 判断数据类型 如加速度、角度等
 * @param: p_data: 数据指针 
 * @param: uiLen: 数据长度 四位
*/
static void CopeWitData(uint8_t ucIndex, uint16_t *p_data, uint32_t uiLen) 
{
    uint32_t uiReg1 = 0, uiReg2 = 0, uiReg1Len = 0, uiReg2Len = 0; // 一个数据包(非一帧包)可能既有加速度又有温度，所以要区分开
    uint16_t *p_usReg1Val = p_data; // 首 把数据包首尾包起来
    uint16_t *p_usReg2Val = p_data+3; // 尾 把数据包首尾包起来
    ...
    switch(ucIndex)
    {
        ...
        case WIT_TIME:  uiReg1 = YYMM;	uiReg1Len = 4;  break; // --------------------------------- // 时间
        case WIT_GYRO:  uiReg1 = GX;  uiLen = 3;break; // ----------------------------------------- // 角速度
        case WIT_MAGNETIC: uiReg1 = HX;  uiLen = 3;break; // -------------------------------------- // 磁力计
    ...
    if(uiLen == 3) // 角速度和磁力计数据长度只有3位而不是4位
    {
        uiReg1Len = 3;
        uiReg2Len = 0;
    }
    if(uiReg1Len)
	  {
		   memcpy(&sReg[uiReg1], p_usReg1Val, uiReg1Len<<1); // p_usReg1Val是uint16_t占2个字节,使用uiReg1Len<<1相当于uiReg1Len*2
		   p_WitRegUpdateCbFunc(uiReg1, uiReg1Len); // 把数据传输完成的标志位s_cDataUpdate置为1
	  }
    if(uiReg2Len)
	  {
...
/**
 * @details: 一次输入一个8位, 累计输入的数据大于等于一帧的长度时会处理数据并把s_cDataUpdate根据协议提示(加速度或角速度等)更新完成
*/
void WitSerialDataIn(uint8_t ucData)
{
    ...
    switch(s_uiProtoclo)
    {
        ...
        case WIT_PROTOCOL_NORMAL:
            ...
            if(s_uiWitDataCnt >= 11) // 一帧正好11位, 如小于11说明一帧还不完整, 直接返回等待下次进入时调用s_uiWitDataCnt++, 直到读了11位开始处理数据
            {
                
                ucSum = __CaliSum(s_ucWitDataBuff, 10); // SUMCRC校验: SUMCRC为char(uint8_t)型，取校验和的低8位
                if(ucSum != s_ucWitDataBuff[10]) // 一帧的第11位为SUMCRC, 检查是否和本地计算的一致
                {
                    s_uiWitDataCnt--;
                    memcpy(s_ucWitDataBuff, &s_ucWitDataBuff[1], s_uiWitDataCnt);
                    return ;
                }
                usData[0] = ((uint16_t)s_ucWitDataBuff[3] << 8) | (uint16_t)s_ucWitDataBuff[2]; // 数据1 数据低8位在前，高8位在后
                usData[1] = ((uint16_t)s_ucWitDataBuff[5] << 8) | (uint16_t)s_ucWitDataBuff[4]; // 数据2
                usData[2] = ((uint16_t)s_ucWitDataBuff[7] << 8) | (uint16_t)s_ucWitDataBuff[6]; // 数据3
                usData[3] = ((uint16_t)s_ucWitDataBuff[9] << 8) | (uint16_t)s_ucWitDataBuff[8]; // 数据4
                CopeWitData(s_ucWitDataBuff[1], usData, 4); // 根据数据内容(加速度或角速度等)保存到sReg对应位置
                s_uiWitDataCnt = 0;
            }
        break;
...
int32_t WitInit(uint32_t uiProtocol, uint8_t ucAddr)
{
    ...
    s_uiProtoclo = uiProtocol; // 确定通信协议, 一般是串口
    s_ucAddr = ucAddr;
    s_uiWitDataCnt = 0;
    return WIT_HAL_OK;
}
